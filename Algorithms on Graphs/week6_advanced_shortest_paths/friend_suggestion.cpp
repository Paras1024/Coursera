...
class Bidijkstra {
    ...
    vector<bool> visitedf; //forward
	vector<bool> visitedb; //backward
public:
    Bidijkstra(int n, Adj adj, Adj cost)
        : n_(n), adj_(adj), cost_(cost), distance_(2, vector<Len>(n, INFINITY)), visitedf(n), visitedb(n)
    { workset_.reserve(n); }
    ...
    Len query(int s, int t) {
		...
		while(!q[0].empty() && !q[1].empty())
		{
			...
			visit(q,0,adj_[0][u][i], distance_[0][u] + cost_[0][u][i]);
			visitedf[u] = true;
			if(visitedf[u] && visitedb[u])
			{
				Len d = INFINITY;
				for(int i=0;i<workset_.size();i++)
				if (distance_[0][workset_[i]] + distance_[1][workset_[i]] < d) 
				d = distance_[0][workset_[i]] + distance_[1][workset_[i]];
			...
			visit(q,1,adj_[1][u][i], distance_[1][u] + cost_[1][u][i]);
			visitedb[u] = true;
			if(visitedf[u] && visitedb[u])
			{
				Len d = INFINITY;
